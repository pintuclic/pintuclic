<template>
  <div class="min-h-screen bg-neutral-lightest text-neutral-dark">
    ...
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      ...
      <div v-else-if="error || !producto" class="mt-6 rounded-card border border-neutral-light bg-neutral-white p-12 text-center shadow-sm"><CircleAlert :size="36" class="mx-auto text-highlight" /><h1 class="mt-3 text-xl font-bold text-corporate">Producto no disponible</h1><p class="mt-2 text-sm text-neutral-medium">{{ error }}</p><router-link to="/catalogo" class="mt-5 inline-flex min-h-11 items-center rounded-button bg-action px-5 text-sm font-medium text-white shadow-sm transition-all hover:bg-action-hover hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action">Volver al catálogo</router-link></div>
      ...
      <template v-else>
        ...
        <section class="mt-5 grid gap-8 rounded-card bg-neutral-white p-4 shadow-sm sm:p-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <div class="mt-3 flex gap-3">
              <button v-for="(imagen, indice) in galeria" :key="imagen" type="button" class="h-16 w-20 overflow-hidden rounded-button border bg-neutral-lightest p-1 transition-all hover:-translate-y-0.5 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" :class="indiceGaleria === indice ? 'border-action' : 'border-neutral-light'" @click="indiceGaleria = indice"><img :src="imagen" alt="" class="h-full w-full object-contain" /></button>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="mt-5 rounded-card border border-neutral-light bg-neutral-white p-4"><button type="button" class="flex min-h-11 w-full items-center justify-between text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" :aria-expanded="descripcionAbierta" @click="descripcionAbierta = !descripcionAbierta"><h2 class="text-sm font-semibold text-corporate">Descripción</h2><ChevronDown :size="17" class="text-neutral-medium transition-transform" :class="descripcionAbierta ? 'rotate-180' : ''" /></button><p v-show="descripcionAbierta" class="mt-2 text-sm leading-6 text-neutral-medium">{{ producto.descripcion || 'Producto de calidad para completar tu proyecto.' }}</p></div>

            <div v-if="colores.length" class="mt-5">
              <div class="flex items-center justify-between gap-3"><h2 class="text-sm font-semibold text-corporate">Elige un color:</h2><button type="button" class="inline-flex min-h-11 items-center gap-1.5 rounded-button px-3 text-xs font-medium text-action transition-colors hover:bg-subaction focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" @click="cartaColoresAbierta = true"><Palette :size="15" /> Ver carta de colores</button></div>
              <div class="mt-2 flex flex-wrap gap-3"><button v-for="(variante, indice) in colores" :key="variante.id_variante" type="button" class="grid h-11 w-11 place-items-center rounded-full border-2 bg-neutral-white transition-all hover:-translate-y-0.5 hover:shadow-md active:translate-y-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" :class="varianteSeleccionadaId === variante.id_variante ? 'border-action ring-2 ring-subaction' : 'border-neutral-light'" :aria-label="`Seleccionar color ${variante.color}`" :title="variante.color ?? ''" @click="seleccionarColor(variante.id_variante)"><span class="h-7 w-7 rounded-full shadow-sm" :class="claseMuestra(indice)" /></button></div>
              <p class="mt-2 text-xs font-medium text-neutral-medium">Seleccionado: <span class="text-corporate">{{ varianteSeleccionada?.color }}</span></p>
            </div>

            <div class="mt-5"><h2 class="text-sm font-semibold text-corporate">Elige un tamaño:</h2><div class="mt-2 grid grid-cols-2 gap-3"><button v-for="variante in presentaciones" :key="variante.id_presentacion" type="button" class="min-h-11 rounded-button border px-4 py-3 text-sm font-medium transition-all hover:-translate-y-0.5 hover:shadow-sm active:translate-y-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" :class="varianteSeleccionada?.id_presentacion === variante.id_presentacion ? 'border-action bg-action text-white' : 'border-neutral-light bg-neutral-white text-neutral-dark hover:border-action hover:text-action'" @click="seleccionarPresentacion(variante.id_presentacion)">{{ variante.presentacion }}</button></div></div>

            <button type="button" class="mt-5 flex min-h-11 w-full items-center justify-between rounded-button border border-neutral-light bg-neutral-white px-4 py-3 text-sm font-medium text-corporate transition-all hover:-translate-y-0.5 hover:border-action hover:bg-subaction hover:shadow-sm active:translate-y-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" @click="calculadoraAbierta = true"><span>Calcular cuánta pintura necesitas</span><Calculator :size="17" /></button>

            <div class="mt-5 grid grid-cols-[auto_1fr] gap-3"><div class="flex min-h-11 items-center rounded-button border border-neutral-light bg-neutral-white"><button type="button" class="grid h-11 w-11 place-items-center transition-colors hover:bg-neutral-lightest focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" aria-label="Reducir cantidad" @click="cantidad = Math.max(1, cantidad - 1)">−</button><span class="min-w-8 text-center text-sm">{{ cantidad }}</span><button type="button" class="grid h-11 w-11 place-items-center transition-colors hover:bg-neutral-lightest focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action" aria-label="Aumentar cantidad" @click="cantidad += 1">+</button></div><button type="button" class="inline-flex min-h-11 items-center justify-center gap-2 rounded-button bg-conversion text-sm font-bold text-white shadow-sm transition-all hover:-translate-y-0.5 hover:bg-conversion-hover hover:shadow-md active:translate-y-0 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-conversion" @click="mostrarMensaje('Agregar al carrito requiere la integración con M07.')"><ShoppingCart :size="17" /> Comprar</button></div>
            <p class="mt-5 flex items-center gap-2 text-xs font-semibold text-conversion-hover"><MapPin :size="14" /> Retiro hoy*</p>
            <p class="mt-2 text-[11px] leading-5 text-neutral-medium">*Los colores mostrados son referenciales y pueden variar según tu pantalla.</p>
          </div>
        </section>
...
const galeria = computed(() => {
  ...
  if (producto.value?.clase_color === 'sin_color') return [obtenerImagenPublicaRespaldo(idProducto.value), GALERIA_PRODUCTO_DEMO.lateral, GALERIA_PRODUCTO_DEMO.ambiente];
...
function seleccionarDesdeCarta(idVariante: number): void { seleccionarColor(idVariante); }
