...
> 6. **Versionado Automatizado y Walkthrough en CADA Implementación:** Tras CUALQUIER cambio o implementación de código, es MANDATORIO incluir en el mensaje del commit la marca `[X.Y.Z]` con la **nueva versión absoluta** del proyecto (patch: sube el tercer dígito; menor: el segundo; mayor: el primero) y generar el Walkthrough de Implementación correspondiente. El bot oficial de GitHub Actions ([`.github/workflows/version-bot.yml`](./.github/workflows/version-bot.yml)) lee el mensaje del último commit, registra esa versión en `docs/CHANGELOG.md`, sube ese cambio con su propio commit y crea el tag y el GitHub Release `vX.Y.Z`. Queda PROHIBIDO editar manualmente el número de versión o `docs/CHANGELOG.md`. Ver [docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md](./docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md).
> 7. **Convención de Commits y Push Controlado:** Los commits deben crearse usando el estándar Conventional Commits con la marca de versión obligatoria (`tipo(modulo): [X.Y.Z] descripcion`, ej: `feat(M04): [3.28.0] implementar registro`), verificando que TypeScript compile limpio y haciendo push únicamente a la rama asignada ([docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_GIT_COMMITS_Y_PUSH.md](./docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_GIT_COMMITS_Y_PUSH.md)).
...
1. **Determinar la Versión:** Marca `[X.Y.Z]` con la **versión nueva** del proyecto: patch (fix) sube el tercer dígito (`3.27.0 → 3.27.1`), menor (feature o mejora) el segundo (`3.27.0 → 3.28.0`) y mayor (cambio grande / breaking) el primero (`3.27.0 → 4.0.0`), según [docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md](./docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md). La marca es la versión absoluta con la que queda el proyecto.
2. **Incluir la Marca en el Commit:** El asunto del commit debe contener la versión entre corchetes (ej: `feat(M01): [3.28.0] implementar consulta publica`). El bot de versionado publica esa versión.
...
2. **Formato del Commit:** Crear commits atómicos utilizando el estándar Conventional Commits con la marca de versión obligatoria (`tipo(modulo): [X.Y.Z] descripcion`, ej: `feat(M04): [3.28.0] implementar registro con verificacion HU-CUE-01` o `fix(M04): [3.27.1] corregir expiracion de tokens`). El bot de versionado registra esa versión en el `CHANGELOG.md` y publica el tag y el Release `vX.Y.Z`.
