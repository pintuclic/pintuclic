...
> 6. **Versionado Automatizado y Walkthrough en CADA Implementación:** Tras CUALQUIER cambio o implementación de código, es MANDATORIO incluir en el mensaje del commit la marca de nivel SemVer relativo `[0.0.1]` (PATCH: fix), `[0.1.0]` (MINOR: feature o mejora) o `[1.0.0]` o mayor (MAJOR: cambio grande / breaking) y generar el Walkthrough de Implementación correspondiente. El bot oficial de GitHub Actions ([`.github/workflows/version-bot.yml`](./.github/workflows/version-bot.yml)) lee el mensaje del último commit, calcula la versión final, actualiza `docs/CHANGELOG.md` y sube ese cambio con su propio commit. Queda PROHIBIDO editar manualmente el número de versión o `docs/CHANGELOG.md`. Ver [docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md](./docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md).
> 7. **Convención de Commits y Push Controlado:** Los commits deben crearse usando el estándar Conventional Commits con la marca de nivel de versión relativo obligatoria (`tipo(modulo): [X.Y.Z] descripcion`, ej: `feat(M04): [0.1.0] implementar registro`), verificando que TypeScript compile limpio y haciendo push únicamente a la rama asignada ([docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_GIT_COMMITS_Y_PUSH.md](./docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_GIT_COMMITS_Y_PUSH.md)).
...
> 11. **Ejecución Obligatoria de ESLint y TypeScript en Backend (`Zero Warnings / Zero Errors`):** Todo agente de IA o desarrollador que cree, modifique o refactorice código en `backend/` DEBE ejecutar obligatoriamente `npm run lint` (ESLint) y `npx tsc --noEmit` en la raíz del backend antes de dar por finalizada la tarea o realizar cualquier commit. El código debe pasar con **0 errores y 0 advertencias** (incluyendo prohibición absoluta de `any` y variables sin uso).
> 12. **Arquitectura de DTOs en Frontend y Prohibición de Validaciones Inline:** Queda TERMINANTEMENTE PROHIBIDO para cualquier desarrollador o Agente de IA declarar esquemas Zod o DTOs inline dentro de componentes o vistas Vue (`.vue`). Todo esquema de validación debe residir en archivos dedicados dentro de carpetas `dtos/`: los DTOs globales (seguridad, contraseñas M20, campos universales) en `frontend/src/core/dtos/`, y los DTOs de formularios o casos de uso específicos en `frontend/src/modules/m[xx]-[nombre]/dtos/`. Las carpetas `interfaces/` se reservan exclusivamente para tipos TypeScript estáticos sin código ejecutable (cero runtime).
...
graph TD
    ...
    K --> L[9. Marca de Versión [X.Y.Z] en el Commit y Generar Walkthrough Oficial]
...
- **Frontend:** Lee el archivo `frontend/infraestructura.md` (o `frontend/README.md`) para estructurar vistas, componentes, llamadas a la API, esquemas Zod en `dtos/` y estilos en el directorio asignado a tu módulo (`src/modules/m[xx]-[nombre]/`). Los DTOs transversales compartidos van en `src/core/dtos/`. Prohibido declarar esquemas Zod inline en componentes `.vue`.
...
- **Origen Centralizado de Datos y Prohibición de Auto-Siembra:** Todo dato inicial, mock o catálogo maestro del sistema reside exclusivamente en `bd/sql/seed_pintuclic.sql` y se gestiona mediante `npm run db:seed` ([`bd/docs/GUIA_MOCKS_Y_DATOS_PRUEBA.md`](./bd/docs/GUIA_MOCKS_Y_DATOS_PRUEBA.md)). Prohibido terminantemente hardcodear arrays de catálogo o crear funciones de auto-siembra en Express (`sembrarPermisos`, etc.). Las carpetas `interfaces/` deben contener única y exclusivamente contratos de compilación (`interface`, `type`), con cero runtime y cero datos.
...
### Paso 9: Marca de Versión en el Commit, Walkthrough y Publicación Automatizada
Al completar la implementación, **DEBES SI O SI**:
1. **Determinar el Nivel SemVer:** Marca `[0.0.1]` para correcciones puntuales (fix), que suben PATCH; `[0.1.0]` para cambios normales (feature o mejora), que suben MINOR; o `[1.0.0]` o mayor para cambios grandes / breaking, que suben MAJOR, según [docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md](./docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md). La marca es un **nivel relativo**, nunca la versión absoluta del proyecto.
2. **Incluir la Marca en el Commit:** El asunto del commit debe contener el nivel entre corchetes (ej: `feat(M01): [0.1.0] implementar consulta publica`). El bot de versionado lee esa marca para calcular la versión final.
...
> 🤖 **CHANGELOG automático:** `docs/CHANGELOG.md` lo escribe el bot de versionado en cada push a `main`/`release`. No lo edites manualmente.
...
2. **Formato del Commit:** Crear commits atómicos utilizando el estándar Conventional Commits con la marca de nivel de versión relativo obligatoria (`tipo(modulo): [X.Y.Z] descripcion`, ej: `feat(M04): [0.1.0] implementar registro con verificacion HU-CUE-01` o `fix(M04): [0.0.1] corregir expiracion de tokens`). La versión absoluta y el `CHANGELOG.md` los calcula el bot de versionado.
