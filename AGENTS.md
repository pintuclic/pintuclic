...
> 6. **Versionado Automatizado y Walkthrough en CADA Implementación:** Tras CUALQUIER cambio o implementación de código, es MANDATORIO incluir en el asunto del commit la marca de nivel SemVer relativo `[0.x.x]` (MINOR: feature, mejora o fix) o `[1.0.0]` o mayor (MAJOR: cambio grande / breaking) y generar el Walkthrough de Implementación correspondiente. El bot oficial de GitHub Actions ([`.github/workflows/version-bot.yml`](./.github/workflows/version-bot.yml)) calcula la versión final, actualiza `docs/CHANGELOG.md`, crea el tag `vX.Y.Z` y publica el Release. Queda PROHIBIDO editar manualmente el número de versión o `docs/CHANGELOG.md`. Ver [docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md](./docs/00_SISTEMA/02_GUIAS_Y_ESTANDARES/GUIA_VERSIONADO_Y_WALKTHROUGHS.md).
...
> 🤖 **CHANGELOG, tag y Release son automáticos:** `docs/CHANGELOG.md` lo escribe el bot en `main` al estabilizar la versión; los tags `vX.Y.Z` y los Releases los publica el bot en `release`/`main`. No los edites ni los crees manualmente.
