...
- **Alcance General:** Incremento **PATCH (v3.36.1)** que reorganiza la arquitectura interna de componentes de M04 bajo los principios SOLID, agrupándolos por subdominios funcionales cohesivos (`auth/`, `registro/`, `recuperacion/`, `perfil/`, `empresas/`, `comunes/`) y proveyendo un barril central de exportación `components/index.ts`.
- **Hitos Clave de Arquitectura:**
